...
import type { RebalanceConfig } from '@/modules/rebalancer-v2'
...
export default function ConfigPanel({ strategy, availableIndices, onChange, mobileOpen, onMobileToggle, maxDate }: Props) {
  ...
  const DEFAULT_REBALANCE: RebalanceConfig = {
    deviationThreshold: 0.10,
    minIntervalMonths: 12,
    tradeCostRate: 0.005,
  }

  const setRebalance = (patch: Partial<RebalanceConfig>) => {
    const current = strategy.rebalanceConfig || DEFAULT_REBALANCE
    onChange({ ...strategy, rebalanceConfig: { ...current, ...patch } })
  }
  ...
  return (
    <aside className={`w-[380px] min-w-[380px] bg-surface border-r border-border overflow-y-auto p-4 flex flex-col gap-4 transition-all max-md:w-full max-md:min-w-0 ${mobileOpen === false ? 'max-md:hidden' : ''} ${mobileOpen ? 'max-md:fixed max-md:inset-0 max-md:z-40 max-md:w-full' : ''}`}>
      ...
      <Card className="border-border bg-card">
        ...
        >
          <span className="text-xs font-semibold text-text-secondary uppercase tracking-wider">再平衡</span>
          <ChevronDown size={14} className={cn('text-text-muted transition-transform', rebalanceOpen && 'rotate-180')} />
        </CardHeader>
        {rebalanceOpen && (
          <CardContent className="px-4 pb-4 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label className="text-xs text-text-secondary">偏离阈值 %</Label>
                <Input
                  type="number" min={1} max={50} step={1}
                  value={Math.round((strategy.rebalanceConfig?.deviationThreshold ?? 0.10) * 100)}
                  onChange={(e) => setRebalance({ deviationThreshold: (Number(e.target.value) || 0) / 100 })}
                  className="h-8 text-sm bg-root border-border font-mono"
                />
              </div>
              <div className="space-y-1">
                <Label className="text-xs text-text-secondary">最小间隔（月）</Label>
                <Input
                  type="number" min={1} max={60} step={1}
                  value={strategy.rebalanceConfig?.minIntervalMonths ?? 12}
                  onChange={(e) => setRebalance({ minIntervalMonths: Number(e.target.value) || 12 })}
                  className="h-8 text-sm bg-root border-border font-mono"
                />
              </div>
            </div>
            <div className="space-y-1">
              <Label className="text-xs text-text-secondary">摩擦成本 %</Label>
              <Input
                type="number" min={0} max={5} step={0.01}
                value={(strategy.rebalanceConfig?.tradeCostRate ?? 0.005) * 100}
                onChange={(e) => setRebalance({ tradeCostRate: (Number(e.target.value) || 0) / 100 })}
                className="h-8 text-sm bg-root border-border font-mono w-24"
              />
            </div>
            <p className="text-[11px] text-text-muted italic">
              在片段卡片中为组合模式片段开启再平衡开关后生效
            </p>
          </CardContent>
        )}
      </Card>
